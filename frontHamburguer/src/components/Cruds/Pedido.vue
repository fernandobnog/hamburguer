...
export default defineComponent({
  ...
  data() {
    return {
      ...
      alertaEstoque: "Trocar item. Sem ",
      verAlertaEstoque: false,
    };
  },
  computed: {
    ...mapState(carneStore, [
  ...
  <div class="card mt-0">
    <div class="flex flex-wrap flex-column align-items-center justify-content-center mt-6">
      <div class="p-inputgroup w-8">
        ...
        <Dropdown v-model="pedido.pessoa" :options="listaPessoa" optionLabel="nome" placeholder="Selecione a pessoa"
          :filter="true" filterPlaceholder="Buscar" class="w-8" />
      </div>
        ...
        <Column>
          <template #body="slotProps">
            <Button icon="pi pi-eye" class="p-button-rounded p-button-success mr-2"
              @click="detalhesPedido(slotProps.data)" />
          </template>
        </Column>
      </DataTable>
    </div>
